...
beforeAll(() => {
    ...
    mockedFs.readFileSync.mockReturnValue('{"TEST_TOKEN": {"es": "Prueba", "en": "Test"}}')
    IntlServer.init({
...
test('Translate missing token', async() => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith('Warn: token SOME_TOKEN not found');
});

test('Sync google sheet', async() => {
    const data = '"TOKEN","es","en"\n"SHEET_TOKEN","token de google sheet","google sheet token"'
    ...
    expect(mockedAxios.get).toHaveBeenCalledWith("https://docs.google.com/spreadsheets/d/someId/gviz/tq?tqx=out:csv&gid=0")
});
