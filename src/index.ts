...
export default class IntlServer implements IntlServerType {
    ...
    updateTranslations(jsonData: any){
        ...
        fs.writeFileSync(this.translationsPath, JSON.stringify(this.translations, null, 2));
    }  
}
...
export async function syncTranslations(): Promise<void> {
    ...
    let response = await axios.get("https://docs.google.com/spreadsheets/d/" + t.sheet_id + "/gviz/tq?tqx=out:csv&gid=0");
    let jsonData = csvToJson(response.data);
    t.updateTranslations(jsonData);
}

export async function translate(token: string, language: string): Promise<string | null> {
    const t = await IntlServer.get();
